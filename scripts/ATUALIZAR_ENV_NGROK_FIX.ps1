# Script para Atualizar .env com URLs do Ngrok e Reiniciar Containers - Versao Corrigida
# Uso: .\ATUALIZAR_ENV_NGROK_FIX.ps1

Write-Host "`n========================================" -ForegroundColor Cyan
Write-Host "ATUALIZACAO AUTOMATICA - NGROK + DOCKER" -ForegroundColor Cyan
Write-Host "========================================`n" -ForegroundColor Cyan

# 1. Obter URLs do arquivo temporario ou API
...
if (Test-Path ".ngrok_urls.tmp") {
    try {
        $urlData = Get-Content ".ngrok_urls.tmp" -Raw | ConvertFrom-Json
        $backendUrl = $urlData.backendUrl
        $frontendUrl = $urlData.frontendUrl
        Write-Host "[OK] URLs lidas do arquivo temporario" -ForegroundColor Green
...
if (-not $backendUrl -or -not $frontendUrl) {
    try {
        ...
        if ($tunnels.tunnels.Count -eq 0) {
            ...
            Write-Host "[INFO] Execute primeiro: .\INICIAR_NGROK_DUAL_FIX.ps1" -ForegroundColor Cyan
            exit 1
        }
...
if (-not $backendUrl -or -not $frontendUrl) {
    ...
    Write-Host "[INFO] Backend: $backendUrl" -ForegroundColor Gray
    Write-Host "[INFO] Frontend: $frontendUrl" -ForegroundColor Gray
    Write-Host "[INFO] Execute: .\INICIAR_NGROK_DUAL_FIX.ps1" -ForegroundColor Cyan
    exit 1
}
...
try {
    $ngrokBackendTest = Invoke-WebRequest -Uri "$backendUrl/health" -Method GET -Headers @{
...
if (Test-Path ".ngrok_urls.tmp") {
    Remove-Item ".ngrok_urls.tmp" -Force
