...
os.environ.setdefault('DATABASE_URL', 'postgresql://postgres:postgres@localhost:5432/hotel_db')
...
async def verificar_inconsistencias():
    ...
    try:
        ...
        print('=== VERIFICACAO DE INCONSISTENCIAS DE STATUS ===')
        print()
        ...
        for reserva in reservas:
            ...
            if reserva.hospedagem:
                ...
                if status_reserva == 'PENDENTE' and status_hospedagem == 'CHECKIN_REALIZADO':
                    ...
                    inconsistencia = 'Reserva CONFIRMADA mas hospedagem ja fez check-in'
                elif status_reserva == 'CHECKED_OUT' and status_hospedagem != 'CHECKOUT_REALIZADO':
                    inconsistencia = 'Reserva CHECKED_OUT mas hospedagem nao CHECKOUT_REALIZADO'
        ...
        if inconsistentes:
            print('ENCONTRADAS', len(inconsistentes), 'RESERVAS INCONSISTENTES:')
            print()
            for i, inc in enumerate(inconsistentes, 1):
                print(f'{i}. Reserva ID: {inc["id"]}')
                print(f'   Codigo: {inc["codigo"]}')
            ...
            print('Todas as reservas estao sincronizadas!')
        ...
        print(f'   Taxa de inconsistencia: {taxa_inconsistencia:.1f}%')
