...
class PontosService:
    ...
    async def validar_reserva_pontos(self, request: ValidarReservaRequest) -> Dict[str, Any]:
        """Validar reserva para lançamento de pontos"""
        try:
            # Verificar se reserva existe e está finalizada
            ...
            # Verificar se cliente corresponde via CPF
            ...
            # Calcular pontos com base em regras vigentes (pontos_regras) e diárias
    ...
    async def confirmar_lancamento(self, request: ConfirmarLancamentoRequest) -> Dict[str, Any]:
        """Confirmar lançamento de pontos"""
        try:
            # Primeiro validar
            ...
            # Creditar pontos de checkout de forma idempotente (origem CHECKOUT)
    ...
    async def gerar_convite(self, request: GerarConviteRequest) -> Dict[str, Any]:
        """Gerar convite para indicação com persistência no banco"""
        try:
            # Verificar se cliente tem pontos suficientes (custo: 50 pontos)
            ...
            convite = await convite_repo.criar_convite(
                ...
                dias_validade=30  # Convite válido por 30 dias
            )
    ...
    async def usar_convite(self, request: UsarConviteRequest) -> Dict[str, Any]:
        """Usar convite de indicação com validação completa"""
        try:
            from app.repositories.convite_repo import ConviteRepository
            from app.core.database import get_db
            ...
            # Creditar pontos para o convidado (bônus: 100 pontos)
            ...
            # Creditar 1 ponto para o convidante
    ...
    def calcular_pontos_reserva(valor_total: float) -> int:
        ...
        Args:
            valor_total (float): Valor total da reserva em reais
            
        Returns:
            int: Quantidade de pontos a serem creditados
        """
        if valor_total <= 0:
            return 0
        ...
        # PON-001 FIX: Log para auditoria
    ...
    async def creditar_pontos_reserva(
        ...
        """Creditar pontos de uma reserva finalizada"""
        try:
